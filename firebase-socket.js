...
export class FirebaseSocket {
  constructor(databaseURL) {
    ...
    this.clientId = localStorage.getItem('websim_client_id');
    if (!this.clientId) {
      this.clientId = Math.random().toString(36).slice(2, 11);
      localStorage.setItem('websim_client_id', this.clientId);
    }
  ...
  async initialize() {
    ...
    let user = (await window.websim.getUser?.()) || { username: "Guest", avatar_url: "" };
    ...
    const isGuest = (user.username || "").startsWith("Guest-") || user.username === "Guest" || user.username === "Wanderer" || (user.username || "").toLowerCase().includes("anonymous");
    if (isGuest) {
  ...
  _getCookieUser() {
    ...
    return {
      username: getC('websim_user_override'),
      avatarUrl: getC('websim_avatar_override')
    };
  }
